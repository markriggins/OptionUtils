 ...
 *   - Type must be exactly: "stock" or "bull-call-spreads"
 *   - RangeName must be a Named Range.
 *     Convenience convention: if RangeName isn't found, the script tries RangeName + "Table"
 *     (e.g., Sheets Table "TslaBullCallSpreads" paired with Named Range "TslaBullCallSpreadsTable").
 *
 * Input tables:
 ...
 *     are ignored.
 *
 * Bull-call-spreads table special handling:
 ...
 *
 * Chart sizing:
 *   - The chart is created only if none exists yet to preserve manual resizing.
 */
...
function plotForSymbol_(ss, symbolRaw, entries) {
  ...
  for (const e of entries) {
    ...
    if (!rangeNameRaw) continue;
    if (type !== "stock" && type !== "bull-call-spreads") continue;
    ...
    if (type === "bull-call-spreads") spreadRanges.push(rng);
  }

  if (missingItems.length > 0 || (stockRanges.length === 0 && spreadRanges.length === 0)) {
    const msg = buildMissingRangeMessage_(ss, symbol, missingItems, stockRanges.length, spreadRanges.length);
  ...
  for (const rng of stockRanges) {
    shares.push(...parseSharesFromTableForSymbol_(rng.getValues(), symbol));
  }

  const spreads = [];
  for (const rng of spreadRanges) {
    spreads.push(...parseSpreadsFromTableForSymbol_(rng.getValues(), symbol));
  }

  // If we parsed nothing, say so clearly (helps debugging)
  if (shares.length === 0 && spreads.length === 0) {
    writeStatus_(sheet,
      `Parsed 0 rows for ${symbol}.\n` +
      `This usually means your headers don't match expected names, or numeric cells are non-numeric.\n\n` +
      `Try:\n` +
      `- Ensure stock table has Shares/Qty + AvgCost/Basis columns\n` +
      `- Ensure spreads table has Long Strike + Short Strike + Contracts + Ave Debit (or Rec Debit)\n` +
  ...
  // ---------- Build output table ----------
  const table = [["Price", "Shares P/L", "Spreads P/L", "Total P/L"]];
  ...
  if (charts.length === 0) {
    ...
    if (!cfg.includeComponentSeries) {
      const priceCol = sheet.getRange(startRow, startCol, table.length, 1);
      const totalCol = sheet.getRange(startRow, startCol + 3, table.length, 1);
...
/* =========================================================
   Portfolios table (creates if missing)
   ========================================================= */
...
function ensureAndReadPortfolios_(ss) {
  ...
  for (let r = 1; r < values.length; r++) {
    ...
    const typ = String(values[r][iType] ?? "").trim();
    const rangeName = String(values[r][iRange] ?? "").trim();

    if (!sym || !typ || !rangeName) continue;
    if (typ !== "stock" && typ !== "bull-call-spreads") continue;
...
function ensureAndReadConfig_(ss, sheet, symbol) {
  const defaults = {
    ...
    chartTitle: `${symbol} Portfolio Value (Unrealized P/L) vs ${symbol} Price`
  };
  ...
  // Named ranges are global => unique per symbol.
...
/**
 * Detect whether a Sheets TABLE exists by probing structured references via the formula engine.
 * Returns true if ROWS(TableName[#ALL]) evaluates to a value (not blank).
 *
 * This detects existence but cannot convert the table to an A1 range without the Advanced API.
 */
function tableExistsByStructuredRefProbe_(ss, tableNameRaw) {
  ...
  const cell = scratchSheet.getRange("ZZ1"); // out of the way
...
function tableRefName_(name) {
  // If it’s a simple identifier, use as-is; else quote it for structured ref formulas.
...
function parseSharesFromTableForSymbol_(rows, symbol) {
  ...
  const idxBasis = findCol_(headerNorm, [
    ...
    "entry", "entryprice", "cost", "purchaseprice"
  ]);

  if (idxQty < 0 || idxBasis < 0) return [];
...
/**
 * Bull-call spread parsing:
 * - Counts ONLY “definition rows” that contain BOTH Long Strike and Short Strike
 * - Ignores fill-detail rows automatically (they usually don’t repeat strikes)
 * - Uses debit cost preference:
 *     1) "Ave Debit" / "Avg Debit" / "Average Debit"
 *     2) "Rec Debit" / "Recommended Debit"
 *     3) "Net Debit" / "Debit" / "Cost" / "Entry"
 * - Requires Contracts (or Qty) column; if missing, defaults qty=1
 */
function parseSpreadsFromTableForSymbol_(rows, symbol) {
  if (!rows || rows.length < 2) return [];
  ...
  // Debit columns
  const idxAveDebit = findCol_(headerNorm, ["avedevit", "avedebit", "avgdebit", "averagedebit"]); // tolerate typos
  const idxRecDebit = findCol_(headerNorm, ["recdebit", "recommendeddebit"]);
  const idxDebitFallback = findCol_(headerNorm, [
    "netdebit", "debit", "cost", "price", "entry", "premiumpaid", "premium"
  ]);
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    // Only “definition rows” (summary rows) have both strikes
    ...
    out.push({ qty, kLong, kShort, debit });
  }
